% Liyan download on 12/31/2015 Thursday
% Usage:
%   >> amari_mjade = Amari_index_ISA(P, ones(1,nS), 'uniform', 2) 
%   where P = Recover*White, ds=[1,1,..,1]->nS, and the last two parameters are as above.
% 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [amari_ind] = Amari_index_ISA(G, ds, weighting_scheme, p)
%function [amari_ind] = Amari_index_ISA(G,ds,weighting_scheme,p)
% 
% Measures the dsxds block-permutation (also called block-scaling) property of the square matrix G, using p-norm. 
% The performance measure is normalized to [0,1]: 
%   o 0 - best (perfect block-permutation matrix), 
%   o 1 - worst case (g^{ij}-s are equal, i.e., their value do not depend on i and j).
%
% INPUT:
%   weighting_scheme: 
%      1)'uniform' or 'subspace-dim-proportional' (see matrix V below). 
%      2)If all the subspace dimensions are equal [ds(1)=ds(2)=...=ds(end)], 
%           then the 2 schemes give the same result.
