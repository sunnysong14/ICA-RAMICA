...
%                 interactive, verbose);
%
% Calculates the PCA matrices for given data (row) TX. Returns
...
% the eigen TX in any ways. Therefore neither does the FASTICA or
...
if N == 1 % if "TX" is vectors, use pcamat()'s original codes.
    ...
    % of this function and pcamat() for matrix X. 07/16/2016.
    covarianceMatrix = cov(TX', 1);
    
elseif N == 2 % if "TX" is real matrices, use my definition.
    TXtX = zeros(nMix, nMix, numSpl);
    
    % Way-1 CovX = 1/p*E[XX']; Do sample-by-sample
    for t = 1 : numSpl
...
%<-- Liyan:: invoke Error after whitening due to the reduce of one value of D
% using $selectedColumns$; the error is because the size of matrix $ICApoj$
% in function FRTestICA1() is not match with others while computing matrix
% $newfea$.
